...
namespace DownloaderForGJSJ
{
    class FileHelper
    {
        ...
        // 读操作
        public static List<string> Read(string fileName, bool omitEmptyLine)
        {
            ...
            try
            {
                ...
                while (!srReadFile.EndOfStream)
                {
                    ...
                    if (omitEmptyLine && strReadLine.Equals(""))
                        continue;
                    //Debug.WriteLine(strReadLine); //屏幕打印每行数据
        ...
        public static string Read(string fileName)
        {
            try
            {
                ...
                {
                    //Method2: 一次全读
                    string result = srReadFile.ReadToEnd();
                    // 关闭读取流文件
                    srReadFile.Close();
                    return result;
                }
        ...
        // 写操作
        public static bool Write(string fileName, List<string> list, ref string errorInfo)
        {
            try
            {
        ...
        // 写操作
        public static bool Write(string fileName, string content, ref string errorInfo)
        {
            try
            {
                FileStream fs = new FileStream(fileName, FileMode.Create, FileAccess.ReadWrite);
        ...
        public static bool ReNameDirectory(string sourceDirName, string destinationDirName, ref string errorMsg)
        {
            try
            {
                if (System.IO.Directory.Exists(sourceDirName))
                {
                    System.IO.DirectoryInfo di = new System.IO.DirectoryInfo(sourceDirName);
                    di.MoveTo(destinationDirName);
                }
                return true;
            }
            catch (Exception e)
        ...
        public static bool ReNameFile(string sourceFileName, string destinationFileName, ref string errorMsg)
        {
            try
            {
        ...
        public static Tuple<bool, string> MoveFolder(string sourcePath, string destPath)
        {
            try
            {
                if (Directory.Exists(sourcePath))
                {
                    if (!Directory.Exists(destPath))
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            System.Diagnostics.Debug.WriteLine("创建目标目录失败：" + ex.Message);
                            return new Tuple<bool, string>(false, "创建目标目录失败：" + ex.Message);
                        }
                    }
                    //获得源文件下所有文件
                    ...
                    foreach (string folder in folders)
                    {
                        ...
                        //采用递归的方法实现
                        if (!MoveFolder(folder, destDir).Item1)
                        {
                            System.Diagnostics.Debug.WriteLine("目录 " + folder + " 不存在");
                            return new Tuple<bool, string>(false, "目录 " + folder + " 不存在");
                        }
                    }
                ...
                else
                {
                    ...
                    return new Tuple<bool, string>(false, "源目录不存在");
                }
            }
            catch (Exception e)
            {
                System.Diagnostics.Debug.WriteLine("MoveFolder失败: " + e.Message);
                return new Tuple<bool, string>(false, "MoveFolder失败: " + e.Message);
            }
